...
func (metrics *ReplicateQueueMetrics) trackRemoveMetric(targetType targetReplicaType, replicaStatus replicaStatus) {
...
func (metrics *ReplicateQueueMetrics) trackRemoveDecommissioningReplicaCount(targetType targetReplicaType) {
...
func (rq *replicateQueue) addOrReplaceVoters(
	...
	if found {
		...
		log.Infof(ctx, "DEBUG! RangeID: %s; Node: %s; Store: %s", repl.RangeID, newVoter.NodeID, newVoter.StoreID)
...
func (rq *replicateQueue) addOrReplaceNonVoters(
	...
	log.Infof(ctx, "in add or replace")
	desc, conf := repl.DescAndSpanConfig()
	...
	log.Infof(ctx, "should be adding here")
...
func (rq *replicateQueue) removeVoter(
	...
	// Remove a replica.
	if !dryRun {
		//log.Infof(ctx, "%s", string(debug.Stack()))
		log.Infof(ctx, "DEBUG! Removing RangeID: %s; Node: %s; Store: %s", repl.RangeID, removeVoter.NodeID, removeVoter.StoreID)
